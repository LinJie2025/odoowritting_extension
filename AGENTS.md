...
| 第三方库 | `lib/xlsx.full.min.js`（SheetJS）。~~`lib/pdf.min.js`+`lib/pdf.worker.min.js`~~ **v3.2 已从 manifest 移除加载**（PDF 修正入口删除；lib 文件保留在仓库未删） |
